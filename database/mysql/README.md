### node-mysql

- [github](https://github.com/felixge/node-mysql)
- [node-mysql-utilities](https://github.com/tshemsedinov/node-mysql-utilities)
- [nodejs-mysql-boilerplate](https://github.com/ocastillo/nodejs-mysql-boilerplate)
- [node-mysql-activerecord](https://github.com/martintajur/node-mysql-activerecord)
- [node-mysql-promise](https://github.com/martinj/node-mysql-promise)
- [TribeDB](https://github.com/yangjiePro/TribeDB) - Node.js 实现的 MySQL 分表分库中间件，用于海量数据的分布式集群储存管理和高并发访问
- [nodejs-MySQL-push-notifications-demo](https://github.com/GianlucaGuarini/nodejs-MySQL-push-notifications-demo)
- [Sequelize和MySQL对照](http://segmentfault.com/a/1190000003987871)